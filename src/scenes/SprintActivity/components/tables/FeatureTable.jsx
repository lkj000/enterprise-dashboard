...
import { feature_lastrun } from "../../requests";
import '../../../global/custom-table.css';


const FeatureTable = ({ input }) => {  

  const data = input?.feature || [];
  ...
  const columns = [
    ...
    {
      name: "story_points",
      label: "Feature Story points",
      options: {
        filterType: 'textField',
        customBodyRender: (value) => {
          return value !== 0 && value !== null ? value : 1;
        },
        customFilterListOptions: { render: v => `Feature Story points: ${v}` },
        setCellProps: () => ({ style: { whiteSpace: 'nowrap', verticalAlign: 'top', textAlign: 'center', paddingLeft: '0px' } }),
    ...
    {
      name: "",
      label: "Linked Issues InProgress (%)",
      options: {
        filterType: 'textField',
  ...
  return (
    <Box>
      ...
      <Box className="table" style={{
        ...
        <MUIDataTable
          title={<span className="custom-table-title">FEATURE</span>}
          data={data}
          columns={columns}
          options={options}
          ...
          components={{
            TableFilterList: (props) => (
              feature_lastrun && <LastUpdatedOnComponent props={props} date={feature_lastrun}/>
            )
          }}
        />
      </Box> }
