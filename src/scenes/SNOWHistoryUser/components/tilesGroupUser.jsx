...
import SnowTable from '../../SNOWHistory/components/SnowTable';
import BarChartUser from '../../../components/BarChartUser';


const TilesGroupUser = ({summary, filter, title, lastRun}) => {
  ...
  const filteredSummaryAssignedTo = filteredSummaryState.filter(item => filter.assignedTo === 'all' || item.assigned_to === filter.assignedTo || (filter.assignedTo === "Not Specified" && !item.assigned_to));
  const filteredSummaryPortfolio = filteredSummaryAssignedTo.filter(item => filter.portfolio === 'all' || item.portfolio === filter.portfolio || (filter.portfolio === "Not Specified" && !item.portfolio));
  ...
  return (
    <Box>
      ...
      <SnowTable summary={filteredSummary} title={title} user={true} lastRun={lastRun} />
    </Box>
  );
};

export default TilesGroupUser;