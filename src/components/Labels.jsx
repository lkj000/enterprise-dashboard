...
import { Box, useTheme, useMediaQuery, List, Divider, ListItem, ListItemButton, ListItemText, Chip, Typography, IconButton, TextField, InputAdornment } from '@mui/material';
...
const Labels = ({list, input}) => {
    ...
    const [jsonData, setJsonData] = useState(list);
    ...
    const handleSearch = (e) => {
        if(e.target.value === ""){
            ...
            setSearch(e.target.value);
            const searchTerm = list_data.filter(item => item.label.toLowerCase().includes(e.target.value.trim().toLowerCase()));
    ...
    //     XLSX.utils.book_append_sheet(wb, ws, list[selectedIndex].label);
    //     const blob = new Blob(
  ...
  return (
    ...
    >
        <DebounceInput
            ...
            endAdornment: search && (
            <IconButton
                onClick={() => {setSearch(""); setJsonData(list); setSelectedIndex(0); setCheckState(0); }}
            ><CloseOutlinedIcon/></IconButton>
            )
            }}
        />
        {checkState === 1 && (<Typography fontSize="16px"> No labels found</Typography>)}
        <Box sx={{
            width: '100%',
            height: tableHeight,
            marginTop: '10px',
