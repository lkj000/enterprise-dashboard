...
$AllSearch = ( "{0}/search/code?q=Docker-Apt" -f $uriBase )
...
for ($i = 0; $i -le 2; i++){
    for ($m = 0; $m -le $numIntiative; $m++){
        $m
        $AllSearch = ( "{0}/search/code?q=Docker-Apt&per_page=100&page={1}&sort=indexed&order={2}" -f $uriBase, $m, $l)
        $ContentsAllSearch = callGithubAPI -urlForGithubAPI $AllSearch
            ...
            if((-not ($RepositoryName -like "esgh-*")) -and (-not ($RepositoryName -like "platform-*"))  -and (-not ($RepositoryName -like "esia*"))){
                if(-not ($Result -like "*$RepositoryName*")){
                    ...
                    :RunnerLabels foreach ($WorkflowsPath in $ContentsAllWorkflows.workflows.path){
                        ...
                        $checkRunsON = Get-Content .\workflow.yaml | Out-String -Stream | Select-String -Pattern "Docker-Apt:"
                        
                        if ($checkRunsON -like "*Docker-Apt:*"){
                            $runnerlabelexcerpt = $checkRunsON -replace "Docker-Apt:", "" -replace "- Docker-Apt:", "" -replace " ", ""
